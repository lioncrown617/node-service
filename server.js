...
function initMqtt() {
  ...
  setTimeout(() => {
    console.log('[MQTT] New day - reconnecting for new topics...');
...
app.get('/odds', async (req, res) => {
  try {
    ...
    return res.json({ ok: false, error: 'Waiting for MQTT data (' + venue + ' R' + raceNo + ')' });
  } catch(e) {
...
app.get('/qin-qpl', async (req, res) => {
  try {
    ...
    const toArr = (odsMap, invMap) =>
      Object.entries(odsMap || {}).map(function(entry) {
        return { combo: entry[0], odds: entry[1], investment: invMap[entry[0]] || 0 };
      });
...
app.listen(PORT, () => {
  console.log('[SERVER] HKJC bridge running on port ' + PORT);
  console.log('[SERVER] /odds        -> WIN+PLA (MQTT only)');
  console.log('[SERVER] /qin-qpl     -> QIN+QPL (MQTT only)');
  console.log('[SERVER] /stream      -> SSE push');
  console.log('[SERVER] /push        -> odd.py push endpoint');
  console.log('[SERVER] /mqtt-status -> cache status');
